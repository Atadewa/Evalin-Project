...
class UjianController extends Controller
{
    public function index()
    {
        ...
        $ujians = Ujian::with('mataPelajaran')
            ...
            ->get();
        $ujians = $ujians->map(function ($ujian, $index) {
            $ujian->no_urut = $index + 1;

            // Check if ujian is finished (past waktu_selesai)
            $ujian->is_finished = Carbon::now()->greaterThan(Carbon::parse($ujian->waktu_selesai));

            // Count soal for this ujian
            $ujian->soal_count = $ujian->soals()->count();
    ...
    {
        $guru = Guru::where('user_id', Auth::id())->firstOrFail();
        $mapels = MataPelajaran::with('kelasMapel')
            ->where('guru_id', $guru->id)
            ->orderBy('nama_mapel')
            ->get();

        $kelasMengajar = $mapels->flatMap(function ($mapel) {
            return $mapel->kelasMapel;
        })->unique('id')->sortBy('nama_kelas')->values();
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'mapel_id' => [
                'required',
                Rule::exists('mata_pelajaran', 'id')->where(function ($query) use ($guru) {
            ...
            'jenis_ujian' => 'required|in:pilihan_ganda,essay,mix',
            'deskripsi' => 'nullable|string',
            'durasi_jam' => 'required|integer|min:0|max:23',
            'durasi_menit' => 'required|integer|min:0|max:59',
            'kelas_id' => ['required', 'array', 'min:1'],
            'kelas_id.*' => [
                'distinct',
                Rule::in(
                    MataPelajaran::with('kelasMapel')
                        ->where('guru_id', $guru->id)
                        ->find($request->mapel_id)
                        ?->kelasMapel
                        ->pluck('id')
                        ->toArray() ?? []
                ),
            ...
            'jadwal.required' => 'Jadwal ujian harus diisi.',
            ...
            'durasi_jam.required' => 'Durasi jam harus diisi.',
        ...
        // Calculate waktu_selesai based on jadwal + duration
        $jadwalCarbon = Carbon::parse($request->jadwal);
        $waktuSelesaiCalculated = $jadwalCarbon->copy()->addMinutes($totalDurasi);

        // Use the calculated waktu_selesai if waktu_selesai is not provided or is before calculated time
        $waktuSelesaiFinal = $request->waktu_selesai ?
            max($request->waktu_selesai, $waktuSelesaiCalculated->format('Y-m-d H:i:s')) :
            $waktuSelesaiCalculated->format('Y-m-d H:i:s');
        ...
        return redirect()->route('guru.soal.create', ['ujian_id' => $ujian->id])
            ->with('success', 'Ujian berhasil dibuat. Silakan tambahkan soal-soal untuk ujian ini.');
    }

    public function show(Ujian $ujian)
    {
        $ujian->load('soals', 'kelasMapel.kelas');
        $ujian->soals = $ujian->soals->map(function ($soal, $index) {
            $soal->no_urut = $index + 1;
            return $soal;
        });
        $hasilujian = UjianSiswa::where('ujian_id', $ujian->id)->get();
        $hasilujian = $hasilujian->map(function ($hasil, $index) {
            $hasil->no_urut = $index + 1;
            return $hasil;
        });
    ...
    public function update(Request $request, Ujian $ujian)
    {
        ...
        if ($ujian->created_by != $guru->id) {
            return redirect()->route('guru.ujian.show', $ujian)->with('error', 'Anda tidak diizinkan mengubah ujian ini.');
        }
        ...
        try {
            ...
            return redirect()->route('guru.ujian.show', $ujian)->with('error', 'Terjadi kesalahan: ' . $th->getMessage());
        }

        return redirect()->route('guru.ujian.show', $ujian)->with('success', 'Ujian berhasil diperbarui.');
    }
    ...
    {
        $ujianSiswa = UjianSiswa::with(['siswa.user', 'jawabanSiswa.soal'])
            ->where('siswa_id', $id)
            ->where('ujian_id', $ujianid)
            ->whereHas('jawabanSiswa', function ($query) use ($ujianid) {
                $query->whereHas('soal', function ($query) use ($ujianid) {
    ...
    // {
    //     $ujian = Ujian::with(['mataPelajaran', 'user'])
    //         ->findOrFail($id);

    //     $hasilUjian = UjianSiswa::with(['siswa.user'])
    //         ->where('ujian_id', $id)
    //         ->get();

    //     return view('guru.ujian.koreksi', compact('ujian', 'hasilUjian'));
    // }
    ...
    // public function konfirmasiNilai(Request $request, $ujianId)
    // {
    //     $guru = Guru::where('user_id', Auth::id())->firstOrFail();
    //     $ujian = Ujian::where('created_by', $guru->id)->findOrFail($ujianId);

    //     $request->validate([
    //         'siswa_ids' => 'required|array',
    //         'siswa_ids.*' => 'exists:ujian_siswa,id'
    //     ]);

    //     UjianSiswa::whereIn('id', $request->siswa_ids)
    //         ->whereHas('ujian', function ($query) use ($guru) {
    //             $query->where('created_by', $guru->id);
    //         })
    //         ->update([
    ...
    //     return redirect()->back()->with('success', 'Nilai berhasil dikonfirmasi dan sekarang dapat dilihat oleh siswa.');
    // }

    // public function batalKonfirmasiNilai(Request $request, $ujianId)
    // {
    //     $guru = Guru::where('user_id', Auth::id())->firstOrFail();
    //     $ujian = Ujian::where('created_by', $guru->id)->findOrFail($ujianId);
    ...
    //     return redirect()->back()->with('success', 'Konfirmasi nilai berhasil dibatalkan.');
    // }
    ...
    {
        try {
            // Validate ujian has at least one question
            if ($ujian->soals()->count() === 0) {
                return response()->json([
                    'success' => false,
                    'message' => 'Ujian harus memiliki minimal 1 soal sebelum dipublikasi.'
                ], 400);
            }

            $ujian->update(['is_published' => true]);

            return response()->json([
                'success' => true,
                'message' => 'Ujian berhasil dipublikasi.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function unpublish(Ujian $ujian)
    {
        try {
            $ujian->update(['is_published' => false]);

            return response()->json([
                'success' => true,
                'message' => 'Ujian berhasil disembunyikan.'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gagal menyembunyikan ujian.'
            ], 500);
        }
    }
}
