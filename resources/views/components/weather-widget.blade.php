...
>
  @if ($weather)
    ...
    <!-- Weather Details -->
    <div>
      <h3 class="font-medium text-gray-900 mb-1">Cuaca Hari Ini</h3>
    ...
    <div class="mt-4 flex items-center justify-between">
      ...
      @php
        ...
        $tempDescription = $temperature ? WeatherTranslator::getTemperatureDescription($temperature) : "Tidak diketahui";
      ...
      <span
        class="inline-flex items-center gap-1 text-xs font-semibold {{ $colorClass }} px-2 py-1 rounded-full"
      >
        <i class="bi {{ $icon }}"></i>
        {{ $tempDescription }}
      </span>
  ...
  @else
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="text-right">
        <div class="text-2xl lg:text-3xl font-bold text-gray-400">--°C</div>
        <div class="text-sm text-gray-500">{{ $city }}</div>
      </div>
    </div>
    ...
    <div>
      <h3 class="font-medium text-gray-900 mb-1">Cuaca Hari Ini</h3>
      <p class="text-sm text-gray-500">Data cuaca tidak tersedia</p>
    </div>
